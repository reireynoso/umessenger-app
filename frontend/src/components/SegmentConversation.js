...
export default ({conversations,socket, conversation, conversation: {messages, users, updatedAt}}) => {
    ...
    const checkTime = () => {
        ...
        // const twoDaysAgo = moment('2020-06-05T19:54:34+0000').fromNow()
        // const twoDaysAgo = moment().fromNow()
        const sameDay = moment(updatedAt).isBefore(Date.now(), "day")
        const twoDaysAgo = moment(today).subtract(2, 'days').calendar()
        // console.log(moment(updatedAt).isAfter(twoDaysAgo))
        // console.log(moment(updatedAt).format('L'))
        // console.log(twoDaysAgo)
        const isAfterTwoDaysAgo = moment(updatedAt).isAfter(twoDaysAgo)
        // console.log(isAfterTwoDaysAgo)
        if(!sameDay){
    ...
    return (
        <div className={checkIfSelected()} onClick={handleConversationSelect}>
            ...
            <div className="segment__info">
                <img className="segment__image" src={users[0].image_url}/>
